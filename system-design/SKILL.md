...
### Core System Design (from Alex Xu's methodology)
...
- [architecture_patterns.md](references/architecture_patterns.md): 9 architecture patterns (Monolithic, Modular Monolith, Microservices, Event-Driven, CQRS, Event Sourcing, Hexagonal, Clean Architecture, API Gateway) with trade-offs and when to use
- [system_design_workflows.md](references/system_design_workflows.md): 6 step-by-step workflows (System Design Interview, Capacity Planning, API Design, Database Schema Design, Scalability Assessment, Migration Planning)
- [tech_decision_guide.md](references/tech_decision_guide.md): 7 decision frameworks (Database Selection, Caching Strategy, Message Queue Selection, Auth Strategy, Frontend Framework, Cloud Provider, API Style)

## Tools

Python scripts for hands-on architecture analysis:

- **Architecture Diagram Generator** (`scripts/architecture_diagram_generator.py`): Scans a project and generates architecture diagrams in Mermaid, PlantUML, or ASCII format. Supports component, layer, and deployment diagram types.
- **Dependency Analyzer** (`scripts/dependency_analyzer.py`): Parses npm, pip, poetry, go, and cargo dependency files. Detects circular dependencies, calculates coupling scores, and flags problematic packages.
- **Project Architect** (`scripts/project_architect.py`): Detects architectural patterns (layered, MVC, hexagonal, clean, microservices), identifies god classes, large files, naming issues, and layer violations.
